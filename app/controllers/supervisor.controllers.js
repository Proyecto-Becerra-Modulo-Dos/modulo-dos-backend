...
// Horas Extra
export const verHorasExtra = async (req, res) => {
  try {
    const [datosDB1] = await basedatos.query(`call sp_ver_empleados();`);

    const [datosDB2] = await pool.query(`call sp_ver_horas_extra();`);
...
export const aprobarHorasExtra = async (req, res) => {
  const id = req.body.id;

  try {
    const [respuesta] = await pool.query(`call sp_aprobar_horas_extra(${id})`);
    res.json(respuesta);
  } catch (error) {
    console.log(error);
    res.status(500).json(error);
...
export const rechazarHorasExtra = async (req, res) => {
  const id = req.body.id;

  try {
    const [respuesta] = await pool.query(
      `call sp_rechazar_horas_extra('${id}')`
    );
    res.json(respuesta);
  } catch (error) {
    console.log(error);
...
export const verHorasExtraAprobados = async (req, res) => {
  try {
    const [datosDB1] = await basedatos.query(`call sp_ver_empleados();`);
...
export const verHorasExtraRechazados = async (req, res) => {
  try {
    const [datosDB1] = await basedatos.query(`call sp_ver_empleados();`);
